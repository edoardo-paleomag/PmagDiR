...
If yes, load file (.csv in the form depth, dec, inc): ")
    ...
    MTF.column <- function(x) {                       #function copying dec correction values
    ...
    repeat {
      ...
      srm.AF.split <- apply(treat, 1, FUN=new.table)      #generate array with a table for any AF steps using the new.table function
      ...
      if(core.TB =="y"){
        ...
        AF0 <- core.TB.filters(AF0)
        AF.last <- core.TB.filters(AF.last)
      }

      NRM <- log10(AF0$int)                                     #NRM intensity on logaritmic scale
      ...
      if (DecOrInc == "i") {
        ...
        if (rnmean == "y") {
          ...
          if (pol.run.mean =="y"){
            ...
            only.N <- roll.mean.plot[!(roll.mean.plot$N==0),]
          }
        }
        if (rnmean=="n") {
          if (N.S.EM == "n") AF.last$N <- ifelse(AF.last$inc>0,1,0)
